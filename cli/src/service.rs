...
use nfo_primitives::Block;
use nfo_runtime::RuntimeApi;
use sc_service::{
...
>, ServiceError> {
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
			let deps = nfo_rpc::FullDeps {
				client: client.clone(),
				pool: pool.clone(),
				select_chain: select_chain.clone(),
				deny_unsafe,
				babe: nfo_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: nfo_rpc::GrandpaDeps {
...
pub fn new_light_base(config: Configuration) -> Result<(
	...
	let light_deps = nfo_rpc::LightDeps {
	...
	let rpc_extensions = nfo_rpc::create_light(light_deps);
